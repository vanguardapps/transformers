...
class GenerationMixin:
    ...
    def _get_logits_processor(
        ...
        # `KNNLogitsProcessor` should always be the second to last logits processor when present
        if generation_config.knn_store is not None:
            processors.append(
                KNNLogitsProcessor(
                    ...
                    interpolation_coefficient=generation_config.knn_interpolation_coefficient,
                    pad_token_id=generation_config.pad_token_id.item(),
                )
            )
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        # if using KNN datastore, always output hidden states
        if generation_config.knn_store is not None:
            output_hidden_states = True
