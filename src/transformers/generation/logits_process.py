...
from .configuration_utils import KNNStore
...
class SequenceBiasLogitsProcessor(LogitsProcessor):
    ...
    def _prepare_bias_variables(self, scores: torch.FloatTensor):
        ...
        # with simpler logic.
        self.length_1_bias = torch.zeros((vocabulary_size,), dtype=torch.float).to(
            scores.device
        )
...
class ForceTokensLogitsProcessor(LogitsProcessor):
    ...
    def __init__(
        self, force_token_map: List[List[int]], _has_warned: Optional[bool] = False
    ):
...
class KNNLogitsProcessor(LogitsProcessor):
    r"""
    Logits processor for use with a `KNNStore` object through which to make predictions using the Fast KNN-MT algorithm.

    # TODO: ROY: Add additional args here from __init__()
    Args:
        datastore (`KNNStore`):
            KNN datastore that is used to perform KNN-MT-style interpolation on the logits during decoding.
        k (`int`):
            The number of top k tokens that are retrieved from the KNN datastore on each decoding step for interpolation
            with base model results.
        temperature (`float`):
            Temperature T to use in calculating interpolated logits from base model logits and top k KNN scores.
    ...
    ```python
    >>> from transformers import AutoTokenizer, AutoModelForCausalLM, KNNStore

    >>> # TODO: ROY: Add definition of dataset for KNNStore training

    >>> tokenizer = AutoTokenizer.from_pretrained("facebook/nllb-200-distilled-600M", src_lang="eng_Latn", tgt_lang="deu_Latn")
    >>> config = AutoConfig.from_pretrained("facebook/nllb-200-distilled-600M", forced_bos_token_id=tokenizer.lang_code_to_id["deu_Latn"])
    >>> model = AutoModelForSeq2SeqLM.from_pretrained("facebook/nllb-200-distilled-600M", config=config)
    ...
    >>> # KNN generation (using informal language bitext as dataset)
    >>> knn_store = KNNStore(embedding_dim=config.hidden_size)
    >>> knn_store.train(dataset)
    >>> out = model.generate(inputs["input_ids"], knn_store=knn_store)
    >>> tokenizer.batch_decode(out, skip_special_tokens=True)[0]
    >>> 'Hallo, wie geht's?'

    >>> # Using KNN generation hyperparameter
    >>> knn_store = KNNStore(embedding_dim=config.hidden_size)
    >>> knn_store.train(dataset)
    >>> out = model.generate(inputs["input_ids"], knn_store=knn_store, knn_interpolation_coefficient=0.8)
    >>> tokenizer.batch_decode(out, skip_special_tokens=True)[0]
    >>> 'Na, wie geht's?'
    ```
    """
    ...
    def __init__(
        ...
        self.k = k
        self.temperature = temperature
        self.interpolation_coeff = interpolation_coefficient

        self.datastore.build_target_datastore(
            encoder_input_ids, encoder_last_hidden_state
        )
    ...
    def __call__(
        ...
        unfinished_sequences: torch.LongTensor = None,
    ) -> torch.FloatTensor:
        model_probs = nn.functional.softmax(scores, dim=-1)

        vocab_dim = scores.shape[1]

        knn_probs = torch.FloatTensor(
            self.datastore.search_target_datastore(
        ...
        probs = (
            self.interpolation_coeff * knn_probs
            + (1 - self.interpolation_coeff) * model_probs
        )
