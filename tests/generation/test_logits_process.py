...
class LogitsProcessorTest(unittest.TestCase):
    ...
    def test_knn_processor(self):
        batch_size = 5
        model_dim = 128
        sequence_len = 20
        vocab_size = 50

        # for KNNLogitsProcessor.__init__()
        encoder_input_ids = torch.randint(
            0,
            vocab_size,
            (batch_size, sequence_len),
        )
        encoder_last_hidden_state = 3 * torch.rand(
            (batch_size, sequence_len, model_dim)
        )
        k = 5
        temperature = 0.3
        interpolation_coefficient = 0.7
        pad_token_id = 1

        # for KNNLogitsProcessor.__call__()
        scores = 3 * torch.rand((batch_size, vocab_size))
        search_probs = nn.functional.softmax(
            torch.rand((batch_size, vocab_size)), dim=-1
        )
        decoder_last_hidden_state = 3 * torch.rand((batch_size, model_dim))
        unfinished_sequences = torch.randint(0, 2, (batch_size,))

        # for assertion
        expected_scores = interpolation_coefficient * search_probs + (
            1 - interpolation_coefficient
        ) * nn.functional.softmax(scores, dim=-1)

        with patch.object(
            KNNStoreSQLite, '__init__', return_value=None
        ) as mock_constructor, patch.object(
            KNNStoreSQLite,
            'build_target_datastore',
            lambda self, encoder_input_ids, encoder_last_hidden_state: None,
        ) as mock_build_target_datastore, patch.object(
            KNNStoreSQLite,
            'search_target_datastore',
            lambda self, decoder_last_hidden_state, k, unfinished_sequences, pad_token_id, return_probs, temperature, vocab_dim: search_probs,
        ) as mock_search_target_datastore:
            datastore = KNNStoreSQLite()
            knn_logits_processor = KNNLogitsProcessor(
            ...
            # calculates new scores by interpolating original scores with search results distribution
            calculated_scores = knn_logits_processor(
                input_ids=None,
                scores=scores,
        ...
        self.assertTrue(
            torch.allclose(calculated_scores, expected_scores, atol=1e-8, rtol=1e-8)
        )
